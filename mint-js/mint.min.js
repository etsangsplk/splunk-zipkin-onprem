...
var factory = function(type, data) {
  ...
  if (type === undefined && data === undefined) {
    ...
    pubsub.publish('mintjs:dto_added', [type, data, dto]);
    dtos.push(dto);
    forwardData('batch');
    return dto;
  }
...
var defaults = {
  ...
  collectResourcePerformance: true,
...
function isCDSUrl (url) {
  var baseURL = getOption('baseURL');
  var hecURL = getOption('hecURL');
...
pubsub.subscribe('mintjs:route_changed', onRouteChange);

function onRouteChange(previous) {
...
var generateV4 = __webpack_require__(9);
...
var _ = __webpack_require__(3);
var baseFormatter = __webpack_require__(11);
var customFormatter = __webpack_require__(25);
...
var timestamp = __webpack_require__(2);
var config = __webpack_require__(1);
var extra = __webpack_require__(6);
...
var baseFormatter = function (type) {
    ...
    if(type !== undefined) {
        return {
            ...
            resourceId: config.getOption('uuid') + '_' + resourceId,
            ...
            referrer: document.referrer ? document.referrer : 'NA',
...
var qwest = __webpack_require__(27);
...
var _ = __webpack_require__(3);
var Lockr = __webpack_require__(28);
var dispatcher = __webpack_require__(14);
var queue = [];
...
var sessions = __webpack_require__(21);
var transactions = __webpack_require__(29);
var timers = __webpack_require__(30);
var extra = __webpack_require__(6);
var formatters = __webpack_require__(10);
var cache = __webpack_require__(15);
var url_blacklist = __webpack_require__(16);
var factory = __webpack_require__(0);
...
function onUnload() {
  pubsub.publish('mintjs:pre_unload', []);
  factory('unload');
}
...
Mint.logEvent = __webpack_require__(42);
...
Mint.logConsole = __webpack_require__(7);
...
var session_id = __webpack_require__(5);
var timestamp = __webpack_require__(2);
var factory = __webpack_require__(0);
var Cookies = __webpack_require__(13);
...
var _ = __webpack_require__(3);
var baseFormatter = __webpack_require__(11);
var LEVELS = __webpack_require__(26);
var config = __webpack_require__(1);
var extra = __webpack_require__(6);
...
var customFormatter = {
  ...
  resource: function (data, type) {
    // if global extra data is set, it will be appended with the extra data for this DTO
    var global_extra_data = extra.getExtraData();
    //selects fields only applicable for resource DTO
    var whitelistedFields = ['performanceStats','loadError','initiatorType','nextHopProtocol', 'workerStart','redirectStart', 'redirectEnd', 'fetchStart', 'domainLookupStart', 'domainLookupEnd', 'connectStart', 'connectEnd', 'secureConnectionStart', 'requestStart', 'responseStart', 'responseEnd', 'transferSize', 'encodedBodySize', 'decodedBodySize', 'name', 'entryType', 'startTime', 'duration'];
    var ndata = _.pick(data, whitelistedFields);
    ndata.extraData = global_extra_data;
    return _.extend(baseFormatter('resource'), ndata);
  },
...
var logHandler = __webpack_require__(35);
var networkHandler = __webpack_require__(36);
var viewHandler = __webpack_require__(37);
var displayHandler = __webpack_require__(38);
var errorHandler = __webpack_require__(39);
var resourcePerformanceHandler = __webpack_require__(40);
var routeChange_handler = __webpack_require__(41);
var config = __webpack_require__(1);
...
var handlers = {
  ...
  collectResourcePerformance: resourcePerformanceHandler,
  collectRouteChanges: routeChange_handler
};
...
var logConsole = __webpack_require__(7);
var config = __webpack_require__(1);
...
function customReadyStateChange(xhr) {
  ...
  if (xhr.readyState == 4) {
    ...
    var networkCall = {
      ...
      url: url,
      startTime: startTime,
      fetchStart: xhr._p_fetchStart,
      endTime: endTime,
      responseEnd: getPerfNow(),
      latency: latency,
...
var attach = function() {
  ...
  if (!perf  || typeof perf.getEntries !== 'function') {
    helpers._log('Mint: window.performance is not supported by browser. Resource performance feature will not be supported!');
    return;
  }
...
var customHandler = function() {
  ...
  setTimeout(function() {
    ...
    var resources = logResources();

    var dataObj = {
      routeChange: false, // indicates if this view is due to soft route change (spa) vs page load
      resources: resources,
      endTimeConcluded: true
    };
    ...
    dataObj = _.extend(dataObj, _t);
    factory('view', dataObj);
  }, 0);
};

function logResources (entries) {
  var addedResources = [];
  entries = entries || perf.getEntriesByType('resource');

  entries.forEach(function (entry) {

    if (config.isCDSUrl(entry.name) === false) {
      var dto = factory('resource', prepareEntry(entry));
      addedResources.push({name: entry.name, resourceId: dto.resourceId});
    }
  });
  return addedResources;
}

function prepareEntry(_entry) {
  var entry = {
    entryType: 'resource',
    name: name,
    responseEnd: _entry.responseEnd,
    initiatorType: _entry.initiatorType,
    fetchStart: _entry.fetchStart,
    duration: _entry.duration,
    performanceStats: _entry
  };
  return entry;
}
...
var oldBufferFullHandler = performance.onresourcetimingbufferfull;
var perf = window.performance || window.msPerformance || window.webkitPerformance || window.mozPerformance;
var handlerAdded = false;
...
function bindEvents() {
  ...
  // only track buffer size if PerformanceObserver is not available
  if(typeof PerformanceObserver !== 'function') {
    perf.onresourcetimingbufferfull = onBufferFull;
  } else {
    //setupPerfObservation();
  }

  if(typeof MutationObserver !== 'function') {
    helpers._log('Mint: MutationObserver not support by browser, resource error reporting will not be supported');
  } else {
...
function unbindEvents() {
  ...
  //tearDownPerfObservation();
  tearDownMutationObservation();
}

function onBufferFull() {
  if(typeof oldBufferFullHandler === 'function') {
    oldBufferFullHandler.apply(null, [].slice.call(arguments));
  }

  helpers._log('Mint: Performance resource buffer is full!');

  // disable Mint code when handler is detached instead of replacing original function. Because custom function may have been replaced.
  if(!handlerAdded || typeof perf.setResourceTimingBufferSize !== 'function') {
    helpers._log('Mint: setResourceTimingBufferSize is not supported!');
    return;
  }

  var totalEntries = perf.getEntries().length;
  var newBufferSize = Math.ceil(Math.max(2 * totalEntries, 150));

  helpers._log('Mint: Performance resource buffer increased: ' + newBufferSize);
  perf.setResourceTimingBufferSize(newBufferSize);
}

// var perfObserverObj;
var mutationObserver = {enabled: false, observer: null};
// function setupPerfObservation() {
//   perfObserverObj = new PerformanceObserver(function(list) {
//     logResources(list.getEntries());
//   });
//   perfObserverObj.observe({entryTypes: ['resource']});
// }
//
// function tearDownPerfObservation() {
//   if(perfObserverObj && typeof perfObserverObj.disconnect === 'function') {
//     perfObserverObj.disconnect();
//   }
// }
...
function setupMutationObservation() {
  ...
  mutationObserver.observer = new MutationObserver(onDomMutation);
  mutationObserver.observer.observe(document.documentElement, config);
  mutationObserver.enabled = true;
}

function tearDownMutationObservation() {
  if(mutationObserver.observer && typeof mutationObserver.observer.disconnect === 'function') {
    mutationObserver.observer.disconnect();
  }
  mutationObserver.enabled = false;
}


var addedResourceMap = {};
var _counter = uuid();
var _uuid = uuid();
function createUID() {
  _counter++;
  return _uuid + '_'+ _counter;
}

function onDomMutation(mutations) {

  mutations.forEach(function(mutation) {
    var addNodes, removeNodes;

    if (mutation.type === 'childList') {
      addNodes = getInterestedResources(mutation.addedNodes);
      removeNodes = getInterestedResources(mutation.removedNodes);
    } else if (mutation.type === 'attributes') {
      addNodes = getInterestedResources([mutation.target]);
      removeNodes = [];
    }

    addNodes.forEach(function(item) {

      item.__muuid = createUID();
      addedResourceMap[item.__muuid] = perf.now();
      item.addEventListener('load', onResourceLoad);
      item.addEventListener('error', onResourceError);
    });

    removeNodes.forEach(function(item) {
      item.removeEventListener('error', onResourceError);
      item.addEventListener('load', onResourceLoad);
    });
...
function onResourceLoad(evt) {
  var t = evt.target;
  t.removeEventListener('load', onResourceLoad);
  var fetchStart = addedResourceMap[t.__muuid] || undefined,
    responseEnd = perf.now(),
    duration = responseEnd - fetchStart;

  var name = t.href || t.src;
  ...
  var entry = {
    ...
    duration: duration,
    performanceStats: getPerfStats(name, 'resource')
  };

  factory('resource', entry);
}

function getPerfStats(name, type) {
  var resources = perf.getEntriesByName(name, type);
  return resources.length ? resources[resources.length-1] : undefined;
}

function onResourceError(evt) {
  var t = evt.target;
  t.removeEventListener('error', onResourceError);
  var fetchStart = addedResourceMap[t.__muuid] || undefined,
      responseEnd = perf.now(),
      duration = responseEnd - fetchStart;
...
function getInterestedResources(nodes) {
  ...
  for(var i=0; i < nodes.length; i++) {
    var node = nodes[i];
    if((node.src || node.href) && node.nodeName !== 'A') {
      res.push(node);
    } else if(node.children && node.children.length > 0) {
      res = res.concat(getInterestedResources(node.children));
    }
  }
...
var loggedViews = [];
var currentView;
var POST_HTTP_DELAY = 100;
var perf = window.performance || window.msPerformance || window.webkitPerformance || window.mozPerformance;
var previousUrl = window.location.href;

function onResourceAdded(type, data, dto) {
  // there are no routeChange events yet, no need to associate resources with views
  if(!currentView) {
    return;
  }

  if(type === 'network' || type === 'resource') {
    addResource(dto, type);
  }
}

function addResource(dto, type) {
  var view,
      dtoStartTime = dto.fetchStart || dto.startTime;

  for(var i=0; i < loggedViews.length; i++) {
    view = loggedViews[i];

    if(view.endTimeConcluded) {
      continue;
    }

    if(view.minStartTime <= dtoStartTime && view.maxEndTime >= dtoStartTime) {
      if(type === 'network') {
        // augment RouteEndTime Boundary
        view.maxEndTime = dto.responseEnd + POST_HTTP_DELAY;
      }
      view.resources.push({name: dto.name || dto.url, resourceId: dto.resourceId});
      view._resources.push(dto);
      return true;
    }
  }
}
...
function proxyMaker(obj, funcName) {
  var originalFunction = obj[funcName];
  return function() {
    if(handlerEnabled) {
      onRouteChange.apply(null, arguments);
...
function setUpProxies() {
  if(!historyObj) {
    helpers._log('Mint: window.history is not available, Route change support will not be available!');
    return;
  }

  historyObj.go = proxyMaker(historyObj, 'go');
  historyObj.pushState = proxyMaker(historyObj, 'pushState');
  historyObj.transitionTo = proxyMaker(historyObj, 'transitionTo');
  historyObj.setState = proxyMaker(historyObj, 'setState');
  historyObj.replaceState = proxyMaker(historyObj, 'replaceState');
}

function bindEvents() {
  setUpProxies();
  pubsub.subscribe('mintjs:dto_added', onResourceAdded);
  window.addEventListener('hashchange', function() {
    onRouteChange();
  });
  pubsub.subscribe('mintjs:pre_unload', flushViews);
}

function unbindEvents() {
  pubsub.unsubscribe('mintjs:dto_added', onResourceAdded);
  window.removeEventListener('hashchange', onRouteChange);
}

function flushViews() {
  var now = window.performance.now();
  loggedViews.forEach(function(view) {
    if(view.maxEndTime <= now) {
      view.endTimeConcluded = true;
    }
    generateViewStats(view);
    factory('view', view);
  });
...
function generateViewStats(view) {
  if(!view._resources) {
    return;
  }

  var responseEnd = view._resources.length ? -1: view.maxEndTime - POST_HTTP_DELAY;
  view._resources.forEach(function(resource) {
    responseEnd = Math.max(resource.responseEnd, responseEnd);
  });
  // remove temp resource key
  delete(view['_resources']);
  view.responseEnd = responseEnd;
  view.loadTime = responseEnd - view.startTime;
}
...
function onRouteChange() {
  ...
  if(currentView) {
    currentView.maxEndTime = now;
    currentView.endTimeConcluded = true;
  }

  setTimeout(function() {
    var currentUrl = window.location.href;
    // only trigger routeHandling if new route is different than previous one.
    if(currentUrl !== previousUrl) {
      createView(true);
      currentView.url = currentUrl;
      pubsub.publish('mintjs:route_changed', [previousUrl]);
      previousUrl = currentUrl;
    }
  }, 30);
}

function createView(isRouteChange) {
  var now = window.performance.now();
  isRouteChange = isRouteChange || false;

  currentView = {
    routeChange: isRouteChange, // indicates if this view is due to soft route change (spa) vs page load
    startTime: now,
    fetchStart: now,
    resources: [],
    _resources: [],
    minStartTime: now - 50,
    maxEndTime: now + POST_HTTP_DELAY,
    endTimeConcluded: false
  };

  loggedViews.push(currentView);
  return currentView;
}
...
if(true) {
  // expose dev only method to hard reset handlers
  window.MintflushViews = flushViews;
}
